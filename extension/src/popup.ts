import type { AnalyzeResponse } from "./definitions";
import type { EmailExtractResult } from "./definitions";

const modeRadios =
  document.querySelectorAll<HTMLInputElement>('input[name="mode"]');
const manualSection = document.querySelector<HTMLDivElement>("#manualSection");
const autoSection = document.querySelector<HTMLDivElement>("#autoSection");
...
const autoAnalyzeButton =
  document.querySelector<HTMLButtonElement>("#autoAnalyzeButton");
...
async function analyzeText(text: string) {
  ...
  if (autoAnalyzeButton) autoAnalyzeButton.disabled = true;
  ...
  try {
    ...
    if (autoAnalyzeButton) autoAnalyzeButton.disabled = false;
  }
}
...
autoAnalyzeButton?.addEventListener("click", async () => {
  try {
    ...
    const results = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: (): EmailExtractResult => {
        function extractEmailData(container: HTMLElement): EmailExtractResult {
    ...
    const emailData = results[0]?.result as EmailExtractResult;
