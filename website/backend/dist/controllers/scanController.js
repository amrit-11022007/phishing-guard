...
async function analyze(req, res) {
    try {
        ...
        const analysis = (0, analyzer_1.analyzeText)(text);
...
async function logScan(req, res) {
    try {
        ...
        const savedLog = await db_1.default.scanLog.create({
            data: {
        ...
        (0, socket_1.broadcastNewScan)(savedLog);
...
async function getLogs(req, res) {
    try {
        ...
        // Fetch from database
        const logs = await db_1.default.scanLog.findMany({
            where: whereClause,
...
async function getStats(req, res) {
    try {
        ...
        const totalScans = await db_1.default.scanLog.count();
        const phishingBlocked = await db_1.default.scanLog.count({
            where: { isPhishing: true },
        });
        const aiCount = await db_1.default.scanLog.count({
            where: { isAI: true },
        });
        const humanCount = await db_1.default.scanLog.count({
            where: { isAI: false },
        });
        // Average AI Confidence
        const aiAvgAgg = await db_1.default.scanLog.aggregate({
            where: { isAI: true },
        ...
        const recentThreats = await db_1.default.scanLog.findMany({
            where: { isPhishing: true },
            orderBy: { timestamp: "desc" },
            take: 5,
        });
        ...
        if (totalScans > 0) {
            const dbLogs = await db_1.default.scanLog.findMany({
                where: {
                    timestamp: {
